...
export = async function main() {
  ...
  const graphNode = await createFargateTask(
    ...
    hostname,
    {
      ...
      extraALBMappings: [
        {
          domain: "graph.decentraland." + envTLD,
          dockerListeningPort: 8020,
          // this server only allows POST which  is outside of what a load balancer can handle, so we react
          // positively to 405 Method Not Allowed
          healthCheck: { matcher: "405" },
        },
        { domain: "graph-play.decentraland." + envTLD, dockerListeningPort: 8000, healthCheck: { matcher: "200-405" } },
      ],
